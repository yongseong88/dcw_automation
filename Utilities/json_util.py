...
class User_api(BaseApi):
    def login_check_api(self, id, pwd):  # 로그인은 POST 방식으로
        ...
        # 'https://dev-account.dotincorp.com/user-app/v1/sites/{self.config['SITE_NO']}-{}/login'.format(
        # readConfig("USER_INFO", "dev_SITE_NO"), readConfig("USER_INFO", "COMP_NO"))
...
class Drive_api(BaseApi):
    ...
    def file_open_api(self, base_url, filekey):
        ...
        if "dev" in base_url:
            file_open_url = "{}/drive-app/v1/dtm/images/{}/device/300/to-dtms".format(base_url, filekey)
        if "dev" not in base_url:
            file_open_url = "{}/drive-app/v1/dtm/images/{}/device/300/to-dtms".format(base_url, filekey)

        file_open_api = requests.get(url=file_open_url)

        # print(f"url값 확인: {file_open_url}")
            ...
            if type(filelist_json[key]) is list:
                for item in filelist_json[key]:
                    ...
                    if item['FILE_NAME'] == file_name and 'D' in item['FILE_KEY']:
                        ...
                        # return folder_in_item['FILE_KEY']
        ...
        for key in filelist_json:
            if type(filelist_json[key]) is list:
                for item in filelist_json[key]:
                    ...
                    # else:
                    #     print(f"{item['FILE_NAME']}은 찾는 파일이 아닙니다.")
                    #     pass
...
class file_group_api():
    ...
    def cloud_group_list_api(self, base_url):
        if "dev" in base_url:
